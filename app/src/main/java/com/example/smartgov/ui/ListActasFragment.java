...
public class ListActasFragment extends Fragment {
    ...
    private void showDetailBottomSheet(int idActa) {
        ...
        addDetailRow(formContainer, "Derivaci├│n", item.has("codigo_barras_seguimiento") ? item.get("codigo_barras_seguimiento").getAsString() : "-");
        addDetailRow(formContainer, "Ubicaci├│n", item.has("codigo_almacen") ? item.get("codigo_almacen").getAsString() : "-");
        addDetailRow(formContainer, "Fecha Guardado", item.get("fecha_hora_guardado").getAsString());
        addDetailRow(formContainer, "Costo Digitalizaci├│n", "S/. " + String.format("%.2f", item.get("costo_digitalizacion").getAsDouble()));
    ...
    private static class ActasAdapter extends RecyclerView.Adapter<ActasAdapter.ViewHolder> {
        ...
        public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
            ...
            holder.textDerivacionUbicacion.setText("Der: " + tracking + " ┬À " + almacen);
